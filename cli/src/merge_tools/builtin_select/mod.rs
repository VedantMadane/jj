...
use crate::merge_tools::builtin_select::term_viewer::TermConflictResolution;
...
pub fn edit_merge_builtin_select(
    ui: &Ui,
    tree: &MergedTree,
    repo_paths: &[&RepoPath],
    ...
    let mut select_tool = SelectTool::new(tree, &matcher)?;
...
impl<'a> SelectTool<'a> {
    pub fn new(tree: &'a MergedTree, matcher: &dyn Matcher) -> BackendResult<Self> {
        let mut conflicts = Vec::new();
        for (path, unsimplified_conflict) in tree.conflicts_matching(matcher) {
            let unsimplified_conflict = unsimplified_conflict?;
            let is_hunk_level = if let Some(file_ids) = unsimplified_conflict.to_file_merge() {
                ...
                // involved, since the hunk would consist of the entire file anyway.
                file_ids.simplify().iter().all(Option::is_some)
            } else {
                false
            };
    ...
    fn view_current_conflict(&mut self) -> BackendResult<()> {
        ...
        let new_state = if conflict.is_hunk_level {
            ...
            .block_on()?
            .expect("should be file conflict");
...
enum ConflictResolution {
    ByTerm(TermConflictResolution),
    ByHunk(HunkConflictResolution),
}

impl ConflictResolution {
    pub fn to_term_resolution(&self) -> Option<&TermConflictResolution> {
        match self {
...
pub fn bstring_to_string_lossy(mut s: BString) -> String {
    let v: Vec<u8> = mem::take(&mut s);
    v.into_string_lossy()
}

pub fn cow_bstr_to_str_lossy(s: Cow<'_, BStr>) -> Cow<'_, str> {
    match s {
        Cow::Borrowed(b) => b.to_str_lossy(),
        Cow::Owned(o) => bstring_to_string_lossy(o).into(),
    }
}
