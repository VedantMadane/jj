...
fn test_build_truncated_evolution_graph() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        truncated_evolution_graph.nodes[commit1.id()].commit_id(),
        commit1.id()
    );
    assert_eq!(
        truncated_evolution_graph.nodes[commit2.id()].commit_id(),
        commit2.id()
    );
    assert_eq!(
        truncated_evolution_graph.nodes[commit3.id()].commit_id(),
        commit3.id()
    );
    assert!(
        truncated_evolution_graph.nodes[commit1.id()]
            .predecessors
            .is_empty()
    );
    assert_eq!(
        truncated_evolution_graph.nodes[commit2.id()].predecessors,
        vec![commit1.id().clone()]
    );
    assert_eq!(
        truncated_evolution_graph.nodes[commit3.id()].predecessors,
        vec![commit1.id().clone()]
    );
...
//        base
//
// Expected result: Solution("baz", parent:X).
#[test]
fn test_automatic_converge_description_parent_and_trees() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let evolution_fork_point = &commit1;
    let evolution_fork_point_parent = &commit_x;
    let solution_parent = &commit_x;
    ...
    let expected_tree = create_merged_tree(vec![
        ...
        (
            ...
            format!(
                ...
                commit3.parents_conflict_label().block_on()?
            ),
        ),
        (
            // ADD C3
    ...
    let _repo = tx.commit("apply solution").block_on().unwrap();
    ...
    assert_eq!(
        applied.tree().path_value(repo_path("file")).unwrap(),
