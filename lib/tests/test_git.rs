...
fn test_import_refs_reimport_conflicted_remote_bookmark() {
    ...
    git::import_refs(tx1.repo_mut(), &import_options).unwrap();
    ...
    git::import_refs(tx2.repo_mut(), &import_options).unwrap();
...
fn test_import_refs_missing_git_commit() {
    ...
    let mut tx = repo.start_transaction();
    let result = git::import_refs(tx.repo_mut(), &import_options);
    assert_matches!(
        result,
...
fn test_import_export_non_tracking_bookmark() {
    ...
    git::import_refs(mut_repo, &default_import_options()).unwrap();
    ...
    git::import_refs(mut_repo, &auto_track_import_options()).unwrap();
    assert!(
        mut_repo
            .view()
            .get_local_bookmark("main".as_ref())
...
#[test]
fn test_fetch_local_remote_conflicts() {
    let test_data = GitRepoData::create();
    let subprocess_options =
        GitSubprocessOptions::from_settings(test_data.repo.settings()).unwrap();
    let import_options = auto_track_import_options();
